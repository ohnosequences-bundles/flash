...
object flashAPI {
  ...
  case class minOverlap(val length: Int)
  extends DefaultFlashOption( Seq("--minOverlap", s"${length}") )

  case class maxOverlap(val length: Int)
  extends DefaultFlashOption( Seq("--maxOverlap", s"${length}") )

  case class threads(val number: Int)
  extends DefaultFlashOption( Seq("--threads", s"${number}") )

  case class readLen(val length: Float)
  extends DefaultFlashOption( Seq("--readLen", s"${length}") )

  case class fragmentLen(val length: Float)
  extends DefaultFlashOption( Seq("--fragment-len", s"${length}") )

  case class fragmentLenStddev(val length: Float)
  extends DefaultFlashOption( Seq("--fragment-len-stddev", s"${length}") )

  case object compress
  extends DefaultFlashOption( Seq("--compress") )

  case object toStdout
  extends DefaultFlashOption( Seq("--to-stdout") )
